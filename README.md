...
- A token is a credential given to us by a backend server so that we may access resources that would otherwise be inaccessible. This is usually given to us through successful authentication/login.
- [ ] What steps can you take in your web apps to keep your data secure?
- We can have two layers of protection once we have a web token. One with protected routes and the other by sending an authentication header with our API calls.
- [ ] Describe how web servers work.
- A web server is a program that can hold data and uses HTTP methods to recieve requests from front end clients. It then sends data back according to the requests allowed by the server endpoints
